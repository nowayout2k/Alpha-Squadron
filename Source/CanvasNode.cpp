...
namespace Engine
{
	...
	CanvasNode::CanvasNode()
		: m_healthBarElement(nullptr), m_healthBgElement(nullptr), m_healthTextElement(nullptr)
	{
		// Create UI elements using unique pointers, then store raw pointers for manipulation.
		auto healthBarElementPtr = std::make_unique<GameSprite>(false, AlphaSquadron::TextureId::MetalBg);
		auto healthBgElementPtr = std::make_unique<GameSprite>(false, AlphaSquadron::TextureId::MetalBg);
		auto healthTextElementPtr = std::make_unique<GameText>(FontId::Arnold,
			"Health",
			18,
			sf::Color::Black,
	...
	void CanvasNode::loadResources()
	{
		...
		m_healthBgElement->setPosition(viewSize.x / 2 - m_healthBgElement->getBoundingRect().width / 2,
			viewSize.y * 0.02f);
		m_healthBarElement->setPosition(
			viewSize.x * 0.001f + (viewSize.x / 2 - m_healthBarElement->getBoundingRect().width / 2),
			viewSize.y * 0.023f);
