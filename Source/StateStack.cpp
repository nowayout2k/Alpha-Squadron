...
namespace Engine
{
	...
	void StateStack::pushState(AlphaSquadron::StateId stateId)
	{
	...
	bool StateStack::handleRolloverKeyEvent(const sf::Event& event)
	{
		...
		if (event.type == sf::Event::KeyPressed &&
			std::find(m_rolloverPressedKeys.begin(), m_rolloverPressedKeys.end(), event.key.code)
				!= m_rolloverPressedKeys.end())
		{
			return true;
		}
	...
	State::Ptr StateStack::createState(AlphaSquadron::StateId stateId)
	{
		// Look up the factory for the given state ID.
		auto found = m_factories.find(stateId);
		if (found == m_factories.end())
		{
			Engine::Debug::logError("Unable to find the state id: " + std::to_string(static_cast<int>(stateId)));
			return nullptr;
		}
	...
	void StateStack::applyPendingChanges()
	{
		...
		for (auto& change : m_pendingList)
		{
			...
			case Clear:
				m_stack.clear();
				AlphaSquadron::ResourceManager::clearAllCache();
				break;
			}
		}
