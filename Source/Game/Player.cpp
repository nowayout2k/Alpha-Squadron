...
{
	std::vector<AlphaSquadron::Player::ActionType> AlphaSquadron::Player::m_realTimeActionTypes;
 ...
 */
	...
	Player::Player(sf::TcpSocket* socket, sf::Int32 identifier, const Engine::KeyBinding* binding)
		: m_keyBinding(binding), m_missionStatus(MissionStatus::None), m_identifier(identifier), m_socket(socket)
	{
		...
		m_actionBinding[ActionType::AccelerateNegX].Action = Engine::DerivedAction<Aircraft>(AircraftMover(-1, 0, identifier));
		m_actionBinding[ActionType::AcceleratePosX].Action = Engine::DerivedAction<Aircraft>(AircraftMover(1, 0, identifier));
		m_actionBinding[ActionType::AccelerateNegY].Action = Engine::DerivedAction<Aircraft>(AircraftMover(0, -1, identifier));
		m_actionBinding[ActionType::AcceleratePosY].Action = Engine::DerivedAction<Aircraft>(AircraftMover(0, 1, identifier));
		m_actionBinding[ActionType::Fire].Action = Engine::DerivedAction<Aircraft>(AircraftFireTrigger(identifier));
		m_actionBinding[ActionType::LaunchMissile].Action = Engine::DerivedAction<Aircraft>(AircraftMissileTrigger(identifier));
	...
	void Player::handleEvent(const sf::Event& event, Engine::CommandQueue& commands)
	{
		...
		if (event.type == sf::Event::KeyPressed)
		{
			...
			if (m_keyBinding && m_keyBinding->checkAction(event.key.code, action) && !isRealtimeAction(action))
			{
				...
				{
					sf::Packet packet;
					packet << static_cast<sf::Int32>(Engine::Client::PlayerEvent);
					packet << m_identifier;
		...
		if ((event.type == sf::Event::KeyPressed || event.type == sf::Event::KeyReleased) && m_socket)
		{
			...
			{
				sf::Packet packet;
				packet << static_cast<sf::Int32>(Engine::Client::PlayerRealtimeChange);
				packet << m_identifier;
				packet << static_cast<sf::Int32>(action);
		...
		if (event.type == sf::Event::KeyPressed && event.key.code == sf::Keyboard::P)
		{
			...
			output.Action = [](Engine::WorldNode& worldNode, sf::Time dt)
			{
		...
		{
			Engine::Debug::log(
				"Window resized to " + std::to_string(event.size.width) + "x" + std::to_string(event.size.height));
		}
		// Toggle debug options on specific key presses.
		if (event.type == sf::Event::KeyPressed && event.key.code == sf::Keyboard::F)
			Engine::Debug::toggleFps();
	...
	bool Player::isRealtimeAction(ActionType actionType)
	{
		...
		return std::any_of(m_realTimeActionTypes.begin(), m_realTimeActionTypes.end(), [actionType](ActionType at)
	...
	void Player::handleRealtimeInput(Engine::CommandQueue& commands)
	{
	...
	void Player::handleRealtimeNetworkInput(Engine::CommandQueue& commands)
	{
	...
	void Player::handleNetworkEvent(ActionType action, Engine::CommandQueue& commands)
	{
