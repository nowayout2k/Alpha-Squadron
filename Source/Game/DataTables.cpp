...
#include "../../Headers/Game/DataTables.h"
#include "../../Headers/Game/AircraftType.h"
#include <fstream>
#include <string>
#include "nlohmann/json.hpp"
#include "../../Headers/Game/Aircraft.h"
...
namespace AlphaSquadron
{
	...
	void SetPickupAction(PickupData& data)
	{
		...
		case PickupType::FireSpread:
			data.Action = [data](Aircraft& a)
			{ a.changeFireSpread(static_cast<int>(data.Value)); };
			break;
		case PickupType::HealthRefill:
			data.Action = [data](Aircraft& a)
			{ a.changeHealth(data.Value); };
			break;
		case PickupType::MissileRefill:
			data.Action = [data](Aircraft& a)
			{ a.changeMissileCount(static_cast<int>(data.Value)); };
			break;
		case PickupType::FireRate:
			data.Action = [data](Aircraft& a)
			{ a.changeFireRate(static_cast<int>(data.Value)); };
			break;
		default:
			break;
		}
	...
	GameData LoadData(const std::string& filename)
	{
		try
		{
			...
			for (const auto& item : j["aircraft"])
			{
				...
				aircraftData.Type = Engine::Utility::stringToAircraftType(item["type"]);
				if (aircraftData.Type == AircraftType::AircraftTypeCount)
				{
					Engine::Debug::logError("Index is greater than length of array! Could not parse " + filename);
					return gameData;
				}

				aircraftData.Health = item["health"];
				aircraftData.Speed = item["maxSpeed"];
				aircraftData.TextureId = Engine::Utility::stringToTextureId(item["textureId"]);
				aircraftData.DespawnDistance = item["aiDespawnDistance"];
				aircraftData.EnterDirection = Engine::Utility::stringToDirection(item["aiEnterDirection"]);
				aircraftData.ExitDirection = Engine::Utility::stringToDirection(item["aiExitDirection"]);
			...
			for (const auto& item : j["pickup"])
			{
				...
				pickupData.Type = Engine::Utility::stringToPickupType(item["type"]);
				pickupData.Value = item["value"];
				pickupData.TextureId = Engine::Utility::stringToTextureId(item["textureId"]);
			...
			for (const auto& item : j["projectile"])
			{
				...
				projectileData.Type = Engine::Utility::stringToProjectileType(item["type"]);
				projectileData.MaxSpeed = item["maxSpeed"];
				projectileData.TextureId = Engine::Utility::stringToTextureId(item["textureId"]);
			...
			for (const auto& item : j["particle"])
			{
				...
				particleData.Type = Engine::Utility::stringToParticleType(item["type"]);
				particleData.Color = Engine::Utility::hexToColor(item["color"]);
