...
{
	Engine::Cache<sf::Texture> ResourceManager::m_textureCache;
	Engine::Cache<sf::SoundBuffer> ResourceManager::m_soundBufferCache;
	Engine::Cache<sf::Font> ResourceManager::m_fontCache;
	Engine::Cache<sf::Shader> ResourceManager::m_shaderCache;

	sf::Font& ResourceManager::loadResource(Engine::FontId fontId)
	{
	...
	sf::Texture& ResourceManager::loadResource(TextureId textureId, sf::Rect<int> area)
	{
		...
		std::string hash = path + std::to_string(area.left) + std::to_string(area.top) + std::to_string(area.width)
	...
	sf::Shader& ResourceManager::loadResource(Engine::ShaderId shaderId)
	{
		// Determine the shader type based on the shader ID and load it.
		auto type = shaderId == Engine::ShaderId::FullPass ? sf::Shader::Type::Vertex : sf::Shader::Type::Fragment;
	...
	std::string ResourceManager::getTexturePath(TextureId textureId)
	{
		...
		default:
			Engine::Debug::logError("Texture Id is unknown!");
			return "../Assets/Textures/UI/ui_sprite_sheet.png";
		}
	}
	...
	std::string ResourceManager::getSoundPath(SoundFxId soundFxId)
	{
		...
		default:
			Engine::Debug::logError("Sound FX Id is unknown!" + std::to_string(static_cast<int>(soundFxId)));
			return "../Assets/SFX/button_click.wav";
		}
	}

	std::string ResourceManager::getFontPath(Engine::FontId fontId)
	{
		// Return the file path corresponding to the given font ID.
		switch (fontId)
		{
		case Engine::FontId::Arnold:
			return "../Assets/Fonts/Arnold.ttf";
		default:
			Engine::Debug::logError("Font Id is unknown!");
			return "../Assets/Fonts/Arnold.ttf";
		}
	}

	std::string ResourceManager::getShaderPath(Engine::ShaderId shaderId)
	{
		// Return the file path corresponding to the given shader ID.
		switch (shaderId)
		{
		case Engine::ShaderId::AddPass:
			return "../Assets/Shaders/Add.frag";
		case Engine::ShaderId::BrightnessPass:
			return "../Assets/Shaders/Brightness.frag";
		case Engine::ShaderId::DownSamplePass:
			return "../Assets/Shaders/DownSample.frag";
		case Engine::ShaderId::GaussianBlurPass:
			return "../Assets/Shaders/GuassianBlur.frag";
		case Engine::ShaderId::FullPass:
