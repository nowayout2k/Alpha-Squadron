...
namespace AlphaSquadron
{
	...
	void Aircraft::changeHealth(float increment)
	{
		...
		if (increment < 0)
		{
			if (((getNodeType() & static_cast<unsigned int>(NodeType::Player)) && m_isDamageAnimationActive)
				|| m_health <= 0)
			{
				m_health = 0;
				destroy();
	...
	void Aircraft::playLocalSound(Engine::CommandQueue& commands, SoundFxId effect, float volume)
	{
		Engine::Command command;
		command.NodeType = static_cast<unsigned int>(NodeType::Sound);
		command.Action = Engine::DerivedAction<Engine::AudioNode>(
			[effect, this, volume](Engine::AudioNode& node, sf::Time dt)
			{
	...
	void Aircraft::handleDamageAnimation(sf::Time deltaTime)
	{
		if (m_isDamageAnimationActive)
		{
			...
			else
			{
				...
				float c = Engine::Utility::lerp(0, 255, t);
				setColor(sf::Color(255, c, c, 255));
			}
		}
	}

	void Aircraft::update(sf::Time deltaTime, Engine::CommandQueue& commands)
	{
		if (isDestroyed())
		{
	...
	void Aircraft::createProjectile(WorldNode& node, ProjectileType projectileType, float xOffset, float yOffset)
	{
		...
		std::unique_ptr<Projectile>
			projectile(new Projectile(isAllied() ? NodeType::AlliedProjectile : NodeType::EnemyProjectile,
	...
	void Aircraft::followAiRoutines(sf::Time deltaTime)
	{
		...
		float radians = Engine::Utility::toRadian(m_aiRoutines[m_routineIndex].Angle);
	...
	void Aircraft::checkProjectileLaunch(sf::Time dt, Engine::CommandQueue& commands)
	{
		if (!isAllied())
		{
			fire();
