...
#define LAUNCH_TIME 0.25f ///< Duration (in seconds) for the projectile's launch phase.
	...
	void Projectile::update(sf::Time deltaTime, Engine::CommandQueue& commands)
	{
		...
		if (isGuided() && !m_isLaunching)
		{
			...
			sf::Vector2f desiredVelocity = Engine::Utility::unitVector(m_targetDirection) * approachRate;
			...
			setRotation(Engine::Utility::toDegree(newAngle));
	...
	void Projectile::guideTowards(sf::Vector2f position)
	{
		if (isGuided())
		{
			...
			m_targetDirection = Engine::Utility::unitVector(position - getWorldPosition());
		}
	}
	...
	void Projectile::loadResources()
	{
		...
		if (isGuided())
		{
			...
			std::unique_ptr<Engine::EmitterNode> smoke(new Engine::EmitterNode(Smoke));
			smoke->setOrigin(0, 0.5f);
			smoke->setPosition(0.f, rect.height / 2);
			attachNode(std::move(smoke));

			std::unique_ptr<Engine::EmitterNode> propellant(new Engine::EmitterNode(Propellant));
