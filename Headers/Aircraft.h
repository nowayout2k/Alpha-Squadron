...
namespace AlphaSquadron
{
	 ...
	 */
 class Aircraft : public Engine::GameSprite
	{
	 public:
		/**
		...
		{
			return GameSprite::getNodeType() | static_cast<unsigned int>(NodeType::Aircraft)
				| static_cast<unsigned int>(m_nodeType);
		}
		 ...
		 */
		virtual void changeFireRate(int increment)
		{
			m_fireRateLevel = std::min(m_fireRateLevel + increment, MAX_FIRE_RATE);
		}
		 ...
		 */
		virtual void changeMissileCount(int increment)
		{
			m_missileCount = std::min(m_missileCount + increment, MAX_MISSILE_COUNT);
		}
		 ...
		 */
		virtual void changeFireSpread(int increment)
		{
			m_spreadLevel = std::min(m_spreadLevel + increment, MAX_SPREAD_LEVEL);
		}
		...
		{
			return m_showExplosion && !m_explosion.isComplete();
		}
		 ...
		 */
		void playLocalSound(Engine::CommandQueue& commands, SoundFxId effect, float volume);
	 ...
	 protected:
		 ...
		 */
		void update(sf::Time dt, Engine::CommandQueue& commands) override;
	 ...
	 private:
		 ...
		 */
		void checkProjectileLaunch(sf::Time dt, Engine::CommandQueue& commands);
		...
		Engine::Command m_fireCommand;              ///< Command to fire bullets.
		Engine::Command m_missileCommand;           ///< Command to launch a missile.
		Engine::Animation m_explosion;              ///< Explosion animation for the aircraft.
