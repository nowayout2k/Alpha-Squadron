...
namespace Engine
{
	...
	class Audio
	{
	 public:
		 ...
		 */
		void playSound(AlphaSquadron::SoundFxId soundFxId, sf::Vector2f position, float volume = 100);
		 ...
		 * @param volume Volume of the sound effect (default is 100).
		 */
		void playSound(AlphaSquadron::SoundFxId soundFxId, float volume = 100);
		 ...
		 */
		void playMusic(AlphaSquadron::MusicId musicType, float volume = 100);
		...
		{
			if (isPaused) m_music.pause(); else m_music.play();
		}
		 ...
		 */
		std::string getMusicPath(AlphaSquadron::MusicId musicId);

		/**
		 * Sets the listener position in 2D space.
		 * @param position Listener's position.
		 */
		static void setListenerPosition(sf::Vector2f position)
		{
