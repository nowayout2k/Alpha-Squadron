...
namespace AlphaSquadron
{
	...
	class SettingsState : public Engine::State
	{
	 public:
		 ...
		 */
		explicit SettingsState(Engine::StateStack& stack, Context context);
	 ...
	 private:
		...
		Engine::Container m_guiContainer;     ///< Container for GUI elements (buttons and labels).
		// Arrays for key binding buttons and labels; there are two sets for two players.
		std::array<Engine::Button::Ptr, 2 * Player::ActionType::ActionCount> m_bindingButtons;
		std::array<Engine::Label::Ptr, 2 * Player::ActionType::ActionCount> m_bindingLabels;
	};
}
