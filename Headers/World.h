...
class World
{
 ...
 private:
	...
	EmptyWorldNode m_worldGraph;           ///< Root node of the world scene graph.
	std::array<WorldNode*, static_cast<int>(Layer::LayerCount)> m_worldLayers{}; ///< Array of world layers.
	...
	CommandQueue m_commandQueue;           ///< Command queue for world actions.
	BloomEffect m_bloomEffect;             ///< Bloom effect for post-processing.
	Audio& m_audioPlayer;                  ///< Reference to the audio system.
	...
	NetworkNode* m_networkNode{};          ///< Pointer to the network node (if any).
	CanvasNode* m_ui{};                    ///< Pointer to the UI node.
