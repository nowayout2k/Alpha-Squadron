...
namespace Engine
{
	...
	class StateStack
	{
	 public:
		 ...
		 */
		template<typename T>
		void registerState(AlphaSquadron::StateId stateId)
		{
		...
		void registerState(AlphaSquadron::StateId stateId, Param arg)
		{
	 ...
	 private:
		...
		struct PendingChange
		{
			 ...
			 */
			PendingChange(StackActionType stackActionType, AlphaSquadron::StateId stateId) : StackActionType(stackActionType), StateId(stateId){};
			/**
			 * @brief Constructs a PendingChange with an action only.
			 * @param stackActionType The type of action.
			 */
			explicit PendingChange(StackActionType stackActionType) : StackActionType(stackActionType), StateId(AlphaSquadron::StateId::None){};
			StackActionType StackActionType; ///< The type of stack action.
			AlphaSquadron::StateId StateId;                 ///< The state identifier associated with the action.
		};
		 ...
		 */
		State::Ptr createState(AlphaSquadron::StateId stateId);
		...
		std::map<AlphaSquadron::StateId, std::function<State::Ptr()>> m_factories; ///< Factories for creating states.
		std::unordered_set<sf::Keyboard::Key>
			m_rolloverPressedKeys;///< Keys that are held down for rollover prevention.
