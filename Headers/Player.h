...
namespace AlphaSquadron
{
	...
	class Player
	{
	 public:
		...
		typedef Engine::LocalPlayerAction::ActionType ActionType;
		 ...
		 */
		Player(sf::TcpSocket* socket, sf::Int32 identifier, const Engine::KeyBinding* binding);
		 ...
		 */
		void handleEvent(const sf::Event& event, Engine::CommandQueue& commands);

		/**
		 * @brief Processes realtime input for the player.
		 * @param commands The command queue to which realtime commands are pushed.
		 */
		void handleRealtimeInput(Engine::CommandQueue& commands);
		 ...
		 */
		void handleRealtimeNetworkInput(Engine::CommandQueue& commands);
		 ...
		 */
		void handleNetworkEvent(ActionType action, Engine::CommandQueue& commands);
	 ...
	 private:
		const Engine::KeyBinding* m_keyBinding;                  ///< Pointer to the player's key binding configuration.
		std::map<ActionType, Engine::Command> m_actionBinding;     ///< Maps action types to corresponding commands.
		...
		std::map<ActionType, bool>
			m_actionProxies;        ///< Stores the state of realtime actions for networked players.
	};
}
#endif //PLAYER_H_
